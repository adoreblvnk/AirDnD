...
const STORAGE_KEY = 'airdnd:detection-allocation:v1';
...
async function main() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => window.__airDnD?.viewer && window.__airDnD?.styleManager,
    ...
    await page.waitForFunction(
      () => window.__airDnD.styleManager.getDetectionDiagnostics()?.observationCount === 5200,
      { timeout: SAMPLE_TIMEOUT_MS, polling: 100 },
    );
    await page.evaluate(() => {
      const viewer = window.__airDnD.viewer;
