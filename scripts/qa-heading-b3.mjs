  ...
  try {
    ...
    await page.evaluate(() => {
      ...
      window.__sampleModelCourse = async function (icao, headingOffsetDeg, windowMs, stepMs = 100) {
        const v = window.__airDnD.viewer;
        const tileset = window.__airDnD.tileset;
        const out = [];
    ...
    await page.evaluate(() => {
      window.__airDnD.styleManager._setDetectionMode('DENSE');
    });
    await sleep(800);
    const shots = [
      ['flights-far-orbitA.png', async () => { /* default follow framing */ }],
      ['flights-far-orbitB.png', () => page.evaluate(() => { window.__airDnD.viewer.camera.rotateRight(1.9); })],
      ['flights-near-orbitA.png', () => page.evaluate(() => {
        const cam = window.__airDnD.viewer.camera;
      ...
      ['flights-near-orbitB.png', () => page.evaluate(() => { window.__airDnD.viewer.camera.rotateLeft(1.2); })],
    ];
    ...
    await page.evaluate(() => {
      window.__airDnD.styleManager._setDetectionMode('OFF');
    });
    ...
    await page.evaluate((hex) => {
      window.__airDnD.dataManager.layers.get('military').module.trackById(hex);
    }, TURN.military[0].hex);
    ...
    if (milModelUp) {
      ...
      for (const [name, move] of [
        ...
        ['military-orbitB.png', () => page.evaluate(() => { window.__airDnD.viewer.camera.rotateRight(1.6); })],
        ...
        ['military-near-orbitB.png', () => page.evaluate(() => {
          window.__airDnD.viewer.camera.rotateRight(1.2);
        })],
      ]) {
        try { await move(); } catch (e) { console.log(`    (camera move for ${name} failed: ${e.message})`); }
        await sleep(1200);
    ...
    const trackFlightsAndWaitModel = async (icao, label) => {
      await page.evaluate((id) => {
        window.__airDnD.dataManager.layers.get('flights').module.trackById(id);
      }, icao);
    ...
    if (await trackFlightsAndWaitModel(TURN.slow.icao, 'slow-turn')) {
      ...
      for (const [name, move] of [
        ...
        ['slowturn-near.png', () => page.evaluate(() => { window.__airDnD.viewer.camera.zoomIn(2500); })],
      ]) {
        try { await move(); } catch (e) { console.log(`    (camera move for ${name} failed: ${e.message})`); }
        await sleep(1200);
        await page.screenshot({ path: path.join(SHOT_DIR_V2, name) });
    ...
    await page.evaluate(async () => {
      ...
      await fl.update(window.__airDnD.viewer); // ingest the first heli fix
    });
    ...
    if (heliTracked) {
      ...
      await page.evaluate(() => {
        window.__airDnD.dataManager.layers.get('flights').module.setParams({ models3d: true });
      });
