...
async function readTileReadiness(page) {
  return page.evaluate(() => {
    ...
    const controller = airdnd.mapStackController;
    const activeStack = controller?.getActiveId?.() || null;
    const tileset = controller?.googleTileset || airdnd.tileset || null;
  ...
  const result = await page.evaluate(async (timeoutMs) => {
    ...
    try {
      while (Date.now() < deadline) {
        airdnd.viewer.scene.render(airdnd.viewer.clock.currentTime);
...
async function capture(page, scenario, frame) {
  ...
  const dataUrl = await page.evaluate(() => (
    window.__airDnD.viewer.scene.canvas.toDataURL('image/png')
  ));
...
async function takeFrameClock(page) {
  const result = await page.evaluate(() => {
    const seam = window.__airDnD.dataManager.layers.get('flights').module.__focusEvidence;
...
async function advanceEvidenceFrame(page, deltaMs = FRAME_MS) {
  return page.evaluate((stepMs) => {
    ...
    const seam = airdnd.dataManager.layers.get('flights').module.__focusEvidence;
...
async function waitForApp(page) {
  ...
  await page.waitForFunction(
    () => Boolean(window.__airDnD?.viewer && window.__airDnD?.dataManager),
...
async function setBasemap(page, stackId) {
  const result = await page.evaluate(async (id) => (
    window.__airDnD.styleManager.setMapStack(id)
  ), stackId);
  if (!result?.ok) {
...
async function enableLayer(page, layerId) {
  await page.evaluate(async (id) => {
    await window.__airDnD.dataManager.setEnabled(id, true);
  }, layerId);
}
...
async function injectAndTrackTarget(page, extraAircraft = [], target = TARGET) {
  ...
  await page.waitForFunction(
    () => Boolean(window.__airDnD.viewer.trackedEntity),
    { timeout: 10_000 },
  );
...
async function readTrackedBasis(page) {
  return page.evaluate((targetId) => {
    const airdnd = window.__airDnD;
    const layer = airdnd.dataManager.layers.get('flights').module;
    ...
    const canvasHeight = airdnd.viewer.scene.canvas.clientHeight;
...
async function flightSnapshot(page) {
  return page.evaluate(() => (
    window.__airDnD.dataManager.layers.get('flights').module.__focusEvidence.snapshot()
  ));
}

async function setTuning(page, params) {
  return page.evaluate((next) => (
    window.__airDnD.dataManager.layers.get('flights').module.__focusEvidence.setTuning(next)
  ), params);
}
...
async function vesselRowsAroundTarget(page) {
  return page.evaluate(() => {
    ...
    const Cartographic = airdnd.viewer.camera.positionCartographic.constructor;
    const ellipsoid = airdnd.viewer.scene.globe.ellipsoid;
    const rows = [];
...
async function runHorizonBand(page, effectiveParams) {
  ...
  await page.evaluate(() => {
    window.__airDnD.dataManager.layers.get('flights').module.stopTracking();
  });
  const contacts = [];
  ...
  await page.evaluate((records) => {
    window.__airDnD.dataManager.layers.get('flights')
