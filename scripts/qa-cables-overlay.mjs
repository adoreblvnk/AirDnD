...
try {
  ...
  await page.evaluate(async () => {
    ...
    for (const [id, entry] of airdnd.dataManager.layers) {
      if (entry.enabled) { try { await airdnd.dataManager.setEnabled(id, false, { origin: 'user' }); } catch { /* measured via counts */ } }
    }
  });
  ...
  if (!control) {
    const labelLatency = await page.evaluate(async (layerId, isLegacy) => {
      ...
      await airdnd.dataManager.setEnabled(layerId, true, { origin: 'user' });
      const deadline = t0 + 60_000;
      const labelsUp = () => {
        if (isLegacy) {
          const stats = airdnd.dataManager.layers.get(layerId)?.module?.getStats?.() || {};
  ...
  if (!legacy && !control) {
    ...
    const cycle = await page.evaluate(async (layerId) => {
      const airdnd = window.__airDnD;
      await airdnd.dataManager.setEnabled(layerId, false, { origin: 'user' });
      airdnd.viewer.scene.requestRender?.();
      await new Promise((r) => setTimeout(r, 1_200));
      const offDiag = window.__airDndWorldOverlay?.getDiagnostics?.() || {};
