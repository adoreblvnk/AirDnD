...
try {
  ...
  await page.waitForFunction(() => !!window.__airDnD?.viewer, { timeout: 90_000 });
  ...
  await page.evaluate(async () => {
    ...
    for (const [id, entry] of airdnd.dataManager.layers) {
      if (entry.enabled) { try { await airdnd.dataManager.setEnabled(id, false, { origin: 'user' }); } catch { /* gate reports via counts */ } }
    }
  });
  ...
  const diag = () => page.evaluate(() => window.__airDnD.getRenderGovernorDiagnostics?.()
    || window.__airDndRenderGovernor?.getDiagnostics?.() || null);
  ...
  const detectionDefault = await page.evaluate(
    () => window.__airDnD.styleManager.getDetectionState?.() || null,
  );
  check(
  ...
  await page.evaluate(() => { window.__airDnD.styleManager._setDetectionMode('OFF'); });
  ...
  await page.evaluate(() => { window.__airDnD.styleManager._setDetectionMode('DENSE'); });
  ...
  const detectPainted = await page.evaluate(() => new Promise((resolve) => {
    const airdnd = window.__airDnD;
    const before = airdnd.styleManager.getDetectionDiagnostics?.()?.frameCount ?? null;
    let paints = 0;
    ...
    let last = airdnd.styleManager.getDetectionDiagnostics?.();
    const remove = airdnd.viewer.scene.postRender.addEventListener(() => {
      const now = airdnd.styleManager.getDetectionDiagnostics?.();
      if (now && now !== last) { paints += 1; last = now; }
    });
    let steps = 0;
    const id = setInterval(() => {
      airdnd.viewer.camera.moveForward(50);
      steps += 1;
      if (steps >= 20) {
        clearInterval(id);
        setTimeout(() => {
          remove();
          resolve({ paints, before, after: airdnd.styleManager.getDetectionDiagnostics?.()?.frameCount ?? null });
        }, 400);
      }
    }, 60);
  }));
  ...
  await page.evaluate(() => { window.__airDnD.styleManager.setStyle('retro'); });
  ...
  await page.evaluate(() => { window.__airDnD.styleManager.setStyle('normal'); });
  ...
  const detectionStillOn = await page.evaluate(
    () => window.__airDnD.styleManager.getDetectionState?.()?.detectionMode || null,
  );
  check(
  ...
  await page.evaluate(async () => {
    await window.__airDnD.dataManager.setEnabled('satellites', true, { origin: 'user' });
  });
  const dSat = await diag();
  check('satellites enable registers its holder', dSat?.holds.includes('satellites'), dSat);
  await page.evaluate(async () => {
    await window.__airDnD.dataManager.setEnabled('satellites', false, { origin: 'user' });
  });
  ...
  await page.evaluate(async () => {
    await window.__airDnD.dataManager.setEnabled('flights', true, { origin: 'user' });
  });
  ...
  await page.evaluate(async () => {
    await window.__airDnD.dataManager.setEnabled('flights', false, { origin: 'user' });
  });
