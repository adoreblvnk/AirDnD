...
async function moveCamera(page, view) {
  return page.evaluate((v) => {
    ...
    const module = airdnd.dataManager.layers.get('traffic').module;
    ...
    try { airdnd.viewer.camera.cancelFlight(); } catch { /* no active flight */ }
    ...
    airdnd.viewer.camera.moveEnd.raiseEvent();
...
async function waitForTrafficCapture(page, beforeLastUpdate, boundaryTime) {
  await page.waitForFunction(
    (before, boundary) => {
      const module = window.__airDnD?.dataManager?.layers?.get('traffic')?.module;
...
async function main() {
  ...
  try {
    ...
    await page.evaluate(async (highView) => {
      const airdnd = window.__airDnD;
      await airdnd.dataManager.restoreEnabledLayerIds([]);
      const ellipsoid = airdnd.viewer.scene.globe.ellipsoid;
      const radians = Math.PI / 180;
      airdnd.viewer.camera.setView({
        destination: ellipsoid.cartographicToCartesian({
      ...
      await airdnd.dataManager.setEnabled('traffic', true);
      airdnd.viewer.camera.changed.raiseEvent();
      airdnd.viewer.camera.moveEnd.raiseEvent();
    }, VIEWS.austin);
    await sleep(500);
    ...
    const timingDiagnostics = await page.evaluate(() => (
      window.__airDnD.dataManager.layers.get('traffic').module.getStats().trafficTiming
    ));
