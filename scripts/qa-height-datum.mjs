 ...
 *      (`airdnd:map-stack-changed` → cctv.js's regime re-arm — see its Task 5
...
async function main() {
  ...
  try {
    ...
    const camCount = await page.evaluate(() => window.__airDnD.dataManager.layers.get('cctv').module.getUIState().count);
    ...
    const allCameras = await page.evaluate(() => window.__airDnD.dataManager.layers.get('cctv').module.getUIState().cameras);
    ...
    async function checkActiveCameraBand(cityLabel, targetCamId, band) {
      await page.evaluate((id) => {
        window.__airDnD.dataManager.layers.get('cctv').module.selectCamera(id, { focus: true, durationSec: 0.1 });
      }, targetCamId);
      await sleep(600);
    ...
    const gotAircraft = await page.waitForFunction(
      () => {
        const mod = window.__airDnD.dataManager.layers.get('flights').module;
    ...
    if (!gotAircraft) {
      ...
      const nearbyList = await page.evaluate(() => {
        const mod = window.__airDnD.dataManager.layers.get('flights').module;
        const center = window.__airDnD.viewer.camera.position;
    ...
    await page.evaluate(async () => {
      await window.__airDnD.mapStackController.setStack('osm');
    });
    ...
    try {
      await page.waitForFunction(() => {
        const p = window.__airDnD?.viewer?.terrainProvider;
    ...
    terrainCtor = await page.evaluate(() => {
      const provider = window.__airDnD.viewer.terrainProvider;
