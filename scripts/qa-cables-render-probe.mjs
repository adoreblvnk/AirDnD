...
try {
  ...
  await page.evaluate(async () => {
    ...
    airdnd.viewer.camera.cancelFlight();
    ...
    for (const [id, entry] of airdnd.dataManager.layers) {
      if (entry.enabled) { try { await airdnd.dataManager.setEnabled(id, false, { origin: 'user' }); } catch { /* probe */ } }
    }
  });
  ...
  const measure = async (label, enabled) => {
    await page.evaluate(async (id, on) => {
      const airdnd = window.__airDnD;
      await airdnd.dataManager.setEnabled(id, on, { origin: 'user' });
    }, LAYER_ID, enabled);
