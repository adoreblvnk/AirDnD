 ...
 *    Drives window.__airDndVoiceCommands.runner(toolName, args) in a headless
 *    page against the dev server and asserts world state via __airDnD /
 *    __airDndAnnotations (camera altitude bands, annotation counts, route
...
async function runRoutingLayer() {
  ...
  const logDir = path.join(ROOT, '.airdnd-logs', 'qa-voice-routing');
...
async function runBehaviorLayer() {
  ...
  try {
    ...
    await page.waitForFunction(
      () => window.__airDnD?.viewer && window.__airDndVoiceCommands?.runner && window.__airDndAnnotations,
      { timeout: 120000, polling: 250 },
    );
    // House rule: the intro flight clobbers teleports issued mid-flight.
    await page.evaluate(() => window.__airDnD.viewer.camera.cancelFlight());

    const run = (name, args) => page.evaluate(
      (n, a) => Promise.resolve(window.__airDndVoiceCommands.runner(n, a))
        .catch((e) => ({ ok: false, error: String(e?.message || e), _threw: true })),
      name, args,
    );
    const camState = () => page.evaluate(() => {
      const c = window.__airDnD.viewer.camera;
    ...
    await settle(2500);
    let annoCount = await page.evaluate(() => window.__airDndAnnotations?.list?.().length ?? window.__airDndAnnotations?.count?.() ?? -1);
    ...
    const ownerTransfer = await page.evaluate(async () => {
      ...
      const runner = window.__airDndVoiceCommands?.runner;
    ...
    await page.waitForFunction(
      () => window.__airDnD.viewer.camera.positionCartographic.height / 1000 > 12000,
    ...
    await page.waitForFunction(
      () => window.__airDnD.viewer.camera.positionCartographic.height / 1000 < 900,
    ...
    for (let i = 0; i < 10 && !alpsOutline; i += 1) {
      ...
      alpsOutline = await page.evaluate(() => {
        ...
        const items = window.__airDndAnnotations?.list?.() || [];
    ...
    const heading = () => page.evaluate(() => window.__airDnD.viewer.camera.heading * 180 / Math.PI);
    ...
    if (r?.ok) {
      ...
      const trackingReleased = await page.evaluate(() => !window.__airDnD.viewer.trackedEntity);
    ...
    if (trackId2 && (await run('track_entity', { query: trackId2, layerId: 'flights' }))?.ok) {
      ...
      await settle(6000);
      const stillTracked = await page.evaluate(() => Boolean(window.__airDnD.viewer.trackedEntity));
    ...
    await settle(800);
    annoCount = await page.evaluate(() => window.__airDndAnnotations?.list?.().length ?? window.__airDndAnnotations?.count?.() ?? -1);
    ...
    const contextEntered = await page.evaluate(
      () => window.__airDnD?.styleManager?.getContextModeState?.() || null,
    );
    ...
    const cockpitBefore = await page.evaluate(
      () => Boolean(window.__airDnD?.styleManager?.cockpitView?.active),
    );
    ...
    const awarenessFlights = await page.evaluate(() => {
      const snap = window.__airDnD?.dataManager?.layers
    ...
    // airliner.
    if (await page.evaluate(() => Boolean(window.__airDnD?.styleManager?.cockpitView?.active))) {
    ...
    const gated = await page.evaluate(
      () => window.__airDnD?.styleManager?.controlCockpit?.('enter') || null,
    );
