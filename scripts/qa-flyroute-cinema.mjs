...
async function installSampler() {
  await page.evaluate(() => {
    ...
    window.__airDndFlyTrace = { rows: [], marks: [] };
    if (window.__airDndFlyTraceRemove) window.__airDndFlyTraceRemove();
    let frame = 0;
    const listener = () => {
      ...
      window.__airDndFlyTrace.rows.push({
        t: performance.now(),
    ...
    window.__airDndFlyTraceRemove = () => viewer.scene.postRender.removeEventListener(listener);
  });
}

async function readTrace() {
  return page.evaluate(() => ({
    rows: window.__airDndFlyTrace.rows.slice(),
    marks: window.__airDndFlyTrace.marks.slice(),
  }));
}
...
try {
  ...
  const run = (name, args = {}) => page.evaluate(
    (n, a) => window.__airDndVoiceCommands.runner(n, a), name, args,
  );
  ...
  const routeInfo = await page.evaluate(() => {
    const route = (window.__airDndAnnotations.list?.() || []).filter((a) => a.type === 'route').at(-1);
  ...
  await page.evaluate(() => window.__airDndFlyTrace.marks.push({ label: 'flight-start', t: performance.now() }));
  ...
  const stillForMs = () => page.evaluate(() => {
    const rows = window.__airDndFlyTrace.rows;
  ...
  await page.evaluate(() => window.__airDndFlyTraceRemove?.());
  ...
  const liveRollDeg = () => page.evaluate(
    () => (window.__airDnD.viewer.camera.roll * 180) / Math.PI,
  );
  let rollBeforeCut = 0;
  ...
  const cut = await page.evaluate(async () => {
    ...
    window.__airDndFlyTrace.marks.push({ label: 'pointerdown', t: performance.now() });
