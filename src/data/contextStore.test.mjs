...
test('selecting a tracking subject stays off the overlay-click event lane', () => {
  withWindow((host) => {
    const seen = [];
    host.addEventListener('airdnd:entity-selected', () => seen.push('selected'));
    host.addEventListener('airdnd:entity-selection-cleared', () => seen.push('cleared'));
    selectTrackedSubjectContext(flightSubject('aaa001'));
    clearTrackedSubjectContext('flights');
    assert.deepEqual(
      seen,
      [],
      'aircraft already publish airdnd:awareness-subject-* — a second lane would make two surfaces fight over one subject',
    );
  });
});
