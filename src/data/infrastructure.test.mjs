...
import { createInfrastructureLayers } from 'airdnd/infrastructure';
import { createLocalGeoJsonLayer } from 'airdnd/infrastructure/geojson';
...
test('package exports import without an application, DOM, fetch, or timers', () => {
  const result = spawnSync(
    ...
    await import('airdnd/infrastructure');
    await import('airdnd/infrastructure/geojson');
    await import('airdnd/infrastructure/lod');
  `,
    ],
...
test('consumer build includes only infrastructure code and resolves assets under a non-root base', async () => {
  ...
  const result = await build({
    ...
    build: {
      ...
      rollupOptions: {
        input: fileURLToPath(
          import.meta.resolve('airdnd/infrastructure'),
        ),
        external: ['cesium'],
