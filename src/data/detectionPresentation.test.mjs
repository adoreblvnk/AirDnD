...
test('detection owns and releases the Cockpit lifecycle listener', async () => {
  ...
  assert.match(source, /addEventListener\('airdnd:cockpit-mode-changed', _cockpitModeListener\)/);
  assert.match(source, /removeEventListener\(\s*'airdnd:cockpit-mode-changed',\s*_cockpitModeListener,?\s*\)/);
  assert.match(
    source,
