...
export class RealtimeTurns {
  ...
  sendTextCommand(text) {
    if (!this.dc || this.dc.readyState !== 'open') {
      throw new Error('AirDnD voice is not connected');
    }
  ...
  async handleRealtimeEvent(event) {
    ...
    if (payload.type === 'error') {
      if (payload.error?.code === 'conversation_already_has_active_response') {
        ...
        this.pendingUserTextResponse = false;
        console.warn('[AirDnD Realtime] Skipped overlapping response.create');
      ...
      if (this.viewport.consumeDeleteError(payload)) {
        console.warn(
          '[AirDnD Realtime] Ignored stale viewport item_not_found',
    ...
    for (const call of calls) {
      ...
      try {
        ...
        result = {
          ok: false,
          error: error?.message || 'AirDnD command failed',
          tool: call.name,
          ...(isRadioFeatureCall
