...
export function responseInstructionForToolResult(result) {
  ...
  if (result?.action === 'control_radio' && result.radioPlaybackRequested) {
    return 'Briefly confirm any other completed AirDnD actions, then say “Turning on the radio.” Do not claim Radio is already playing.';
  }
  if (result?.action === 'get_entity_context') {
    ...
    return [
      'Answer the user naturally using the returned AirDnD entity context.',
  ...
  if (result?.action === 'get_current_view_state') {
    return 'Briefly summarize the current AirDnD camera, active style, and relevant enabled layers. Do not repeat yourself.';
  }
  ...
  return 'Briefly confirm the completed AirDnD action once. Do not repeat yourself.';
}
