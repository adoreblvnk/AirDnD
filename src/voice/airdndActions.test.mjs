...
import {
  ...
  createAirdndActionRunner as createActionRunner,
  cctvVoiceFocusOutcome,
  formatTrackedEntityLabel,
  knownRadioLocation,
  normalizeStackId,
} from './airdndActions.js';
...
test('every live basemap is reachable by its own id — no enum value without a voice alias', () => {
  ...
  const enumIds = AIRDND_REALTIME_TOOLS.find(tool => tool.name === 'set_map_stack').parameters.properties.stack.enum;
  assert.deepEqual(
    [...enumIds].sort(),
...
test('nearest-aircraft voice action serializes layer enable, arrival, refresh, airborne query, and selection', async () => {
  ...
  const runner = createAirdndActionRunner({ viewer, styleManager, dataManager });
...
test('invalid named voice navigation never releases the current camera owner', async () => {
  ...
  const invalidRouteRunner = createAirdndActionRunner({
    viewer,
    styleManager,
    dataManager: { layers: new Map(), isEnabled: () => false, getAll: () => [] },
    annotations: { list: () => [{
  ...
  const outOfRangeRouteRunner = createAirdndActionRunner({
    viewer,
    styleManager,
    dataManager: { layers: new Map(), isEnabled: () => false, getAll: () => [] },
    annotations: { list: () => [{
...
test('generic layer visibility exposes lifecycle truth for every manager phase and thrown failure', async () => {
  ...
  const rejectedRunner = createAirdndActionRunner({ viewer, styleManager: {}, dataManager: rejectedManager });
  ...
  const flightsRunner = createAirdndActionRunner({ viewer, styleManager: {}, dataManager: flightsManager });
  ...
  const missingRadioRunner = createAirdndActionRunner({
    viewer,
    styleManager: {},
 ...
 * `createAirdndActionRunner` to install its listeners, with every pick under the
 * test's control.
 */
...
function analystRunner(awareness) {
  ...
  return createAirdndActionRunner({
    viewer,
    styleManager: {},
    dataManager: {
...
test('general satellite pass resolves once, refuses ambiguity, and preserves ISS call semantics', async () => {
  ...
  const runner = createAirdndActionRunner({ viewer, styleManager: {}, dataManager: { layers: new Map([['satellites', { module: layer }]]) } });
