 ...
 *    a chip label into a permanent two-child shell: a `.airdnd-flap-text` span
 *    holding one long-lived `Text` node, and an `aria-hidden`
 *    `.airdnd-flap-cells` sibling. After that the ONLY text operation for the
 ...
 *    staggered via a per-cell `--airdnd-flap-delay` custom property. A change
 ...
 * `airdnd-flap-out`/`airdnd-flap-in` keyframe crossover in style.css — it is what
 * makes an interrupted cascade pick honest outgoing glyphs.
 */
export const FLAP_TURN_RATIO = 0.5;

const HOST_CLASS = 'airdnd-flap-host';
const ACTIVE_CLASS = 'airdnd-flap-active';
const TEXT_CLASS = 'airdnd-flap-text';
const CELLS_CLASS = 'airdnd-flap-cells';
const CELL_CLASS = 'airdnd-flap-cell';
const FLAPPING_CLASS = 'is-flapping';
const SIZING_CLASS = 'airdnd-flap-sizing';
 ...
 * `.airdnd-flap-text` is the same object for the life of the chip, so a label
...
function clearSizing(element) {
  ...
  element.style?.removeProperty('--airdnd-flap-total');
}
...
function easeWidth(element, fromWidth, toWidth, durationMs) {
  ...
  element.style.setProperty('--airdnd-flap-total', `${durationMs}ms`);
...
function rest(element, host) {
  ...
  element.style?.removeProperty('--airdnd-flap-dur');
...
export function setSplitFlapText(element, text, options = {}) {
  ...
  for (const cell of plan.cells) {
    ...
    if (cell.changed) {
      ...
      node.style.setProperty('--airdnd-flap-delay', `${cell.delayMs}ms`);
    }
    cellNodes.push(node);
  }
  ...
  element.style.setProperty('--airdnd-flap-dur', `${charMs}ms`);
...
export function disposeSplitFlap(element) {
  ...
  cancelWidthEase(element);
  clearSizing(element);
  const cells = element.querySelector?.('.airdnd-flap-cells');
