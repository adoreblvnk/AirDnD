...
const RATE_LIMIT_FIELDS = [
  'AIRDND_RATELIMIT_OPENAI_PER_MIN',
...
test('Pinokio start has one fail-closed launcher process', () => {
  ...
  assert.equal(
    ...
    '{{env.PINOKIO_SHARE_VAR || "__airdnd_sharing_disabled__"}}',
  );
});
...
test('Pinokio menu resolves the nested install marker and exposes each lifecycle state', async (t) => {
  const fixture = await mkdtemp(path.join(os.tmpdir(), 'airdnd-pinokio-menu-'));
  ...
  assert.deepEqual(await render({
    ...
    { text: "Open AirDnD", href: 'http://127.0.0.1:4173/', default: true },
...
test('Pinokio install recognizes direct execution through a linked app directory', async (t) => {
  const fixture = await mkdtemp(path.join(os.tmpdir(), 'airdnd-pinokio-entry-'));
  t.after(() => rm(fixture, { recursive: true, force: true }));
...
test('Pinokio direct execution fallback remains exact and Update-safe', () => {
  const missing = path.join(os.tmpdir(), 'airdnd-missing-pinokio-install.mjs');
  const differentMissing = path.join(os.tmpdir(), 'airdnd-other-missing-pinokio-install.mjs');
...
test('Pinokio start enters the canonical app root before loading Vite', async (t) => {
  const fixture = await mkdtemp(path.join(os.tmpdir(), 'airdnd-pinokio-root-'));
