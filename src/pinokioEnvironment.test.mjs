...
test('the fresh template keeps provider credentials out of native Configure', () => {
  ...
  assert.equal(configured.PINOKIO_SHARE_VAR, '__airdnd_sharing_disabled__');
  assert.equal(configured.AIRDND_RATELIMIT_OPENAI_PER_MIN, '30');
  assert.equal(configured.AIRDND_RATELIMIT_GOOGLE_PER_MIN, '120');
...
test('raw app-file values override Pinokio-global values, including blanks', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'airdnd-pinokio-env-'));
  try {
    const filepath = path.join(root, 'ENVIRONMENT');
    writeFileSync(filepath, [
      'GOOGLE_MAPS_API_KEY=app-configured',
      'OPENAI_API_KEY=',
      'AIRDND_RATELIMIT_OPENAI_PER_MIN=45',
      'AIRDND_RATELIMIT_GOOGLE_PER_MIN=',
      'PINOKIO_SHARE_CLOUDFLARE=false',
      'PINOKIO_SHARE_LOCAL=false',
      'PINOKIO_SHARE_VAR=__airdnd_sharing_disabled__',
      '',
    ].join('\n'));
    const environment = {
      ...
      AIRDND_RATELIMIT_OPENAI_PER_MIN: '999',
      AIRDND_RATELIMIT_GOOGLE_PER_MIN: '999',
    ...
    assert.equal(environment.AIRDND_RATELIMIT_OPENAI_PER_MIN, '45');
    assert.equal(environment.AIRDND_RATELIMIT_GOOGLE_PER_MIN, '');
    assert.equal(environment.PINOKIO_SHARE_CLOUDFLARE, 'false');
    assert.equal(environment.PINOKIO_SHARE_LOCAL, 'false');
    assert.equal(environment.PINOKIO_SHARE_VAR, '__airdnd_sharing_disabled__');
...
test('an existing Pinokio file gains the canonical non-secret sharing boundary', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'airdnd-pinokio-env-legacy-'));
  try {
    const filepath = path.join(root, 'ENVIRONMENT');
    ...
    assert.equal(environment.AIRDND_RATELIMIT_OPENAI_PER_MIN, '30');
    assert.equal(environment.AIRDND_RATELIMIT_GOOGLE_PER_MIN, '120');
    assert.equal(environment.PINOKIO_SHARE_LOCAL, 'false');
    assert.equal(environment.PINOKIO_SHARE_VAR, '__airdnd_sharing_disabled__');
    assert.equal(environment.PINOKIO_SHARE_PASSCODE, '');
    const persisted = readFileSync(filepath, 'utf8');
    assert.match(persisted, /^PINOKIO_SHARE_LOCAL=false$/m);
    assert.match(persisted, /^PINOKIO_SHARE_VAR=__airdnd_sharing_disabled__$/m);
    assert.match(persisted, /^OPENAI_API_KEY=app-value$/m);
    assert.doesNotMatch(persisted, /^AIRDND_RATELIMIT_OPENAI_PER_MIN=/m);
    assert.doesNotMatch(persisted, /^AIRDND_RATELIMIT_GOOGLE_PER_MIN=/m);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
...
test('blank and duplicate sharing controls are canonicalized before Pinokio re-reads them', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'airdnd-pinokio-env-duplicates-'));
  try {
    const filepath = path.join(root, 'ENVIRONMENT');
    const providerLines = [
...
for (const fixture of [
  ...
  test(`${fixture.name} Pinokio configuration preserves provider values during migration`, () => {
    const root = mkdtempSync(path.join(tmpdir(), 'airdnd-pinokio-env-utf16-'));
    try {
      const filepath = path.join(root, 'ENVIRONMENT');
...
test('server Google key follows app values, blanks and absence instead of inherited credentials', () => {
  const root = mkdtempSync(path.join(tmpdir(), 'airdnd-pinokio-server-key-'));
  try {
    const filepath = path.join(root, 'ENVIRONMENT');
