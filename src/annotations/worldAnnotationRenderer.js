...
export function createWorldAnnotationRenderer(viewer) {
  const dataSource = new Cesium.CustomDataSource('airdnd-annotations');
  viewer.dataSources.add(dataSource);

  // Register the AirDnDRouteFlow fabric once so Cesium's Material.fromType() can build the
...
/** Register the AirDnDRouteFlow fabric ONCE so Cesium's `Material.fromType('AirDnDRouteFlow')`
...
export function ensureFlowFabricRegistered() {
  ...
  makeRouteFlowMaterial('#ffffff'); // side effect: registers the 'AirDnDRouteFlow' type
  _flowFabricRegistered = true;
}

/**
 * MaterialProperty for the animated route. Cesium builds the rendered Material once
 * from getType() (our registered AirDnDRouteFlow fabric), then EACH FRAME calls
