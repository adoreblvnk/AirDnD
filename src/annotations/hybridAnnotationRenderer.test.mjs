...
function findAnnotationGroup(document) {
  const layer = document.body.children.find((child) => child.classList.contains('airdnd-screen-whiteboard'));
  const svg = layer.children.find((child) => child.classList.contains('airdnd-screen-whiteboard-svg'));
  return {
    svg,
    group: svg.children.find((child) => child.classList.contains('airdnd-anno')),
  };
}
...
function annotationGroups(svg) {
  return svg.children.filter((child) => child.classList.contains('airdnd-anno'));
}
...
test('hybrid outline upgrade preserves the screen group and adds world geometry', (t) => {
  ...
  const originalCallout = before.group.querySelector('.airdnd-anno-callout');
  const originalDot = before.group.querySelector('.airdnd-anno-dot');
  assert.equal(before.group.querySelectorAll('.airdnd-anno-ring').length, 2);
  ...
  assert.equal(after.svg.children.filter((child) => child.classList.contains('airdnd-anno')).length, 1);
  assert.equal(after.group.querySelectorAll('.airdnd-anno-ring').length, 0, 'screen reticle rings are removed');
  assert.equal(after.group.querySelector('.airdnd-anno-callout'), originalCallout, 'the callout node is retained');
  assert.equal(after.group.querySelector('.airdnd-anno-dot'), originalDot, 'the anchor dot is retained');
