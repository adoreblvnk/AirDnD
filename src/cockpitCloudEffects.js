...
const WEATHER_ENABLED_STORAGE_KEY = 'airDnD.cockpitWeatherEffects.enabled';
...
export class CockpitCloudEffectsController {
  constructor(viewer, { weatherService = applicationServices.weather } = {}) {
    ...
    window.addEventListener('airdnd:cockpit-mode-changed', this.onCockpitMode);
    window.addEventListener('airdnd:cockpit-weather-toggle', this.onEnabledChange);
  ...
  emitEnabledState() {
    window.dispatchEvent(
      new CustomEvent('airdnd:cockpit-weather-state', {
  ...
  destroy() {
    ...
    window.removeEventListener('airdnd:cockpit-mode-changed', this.onCockpitMode);
    window.removeEventListener(
      'airdnd:cockpit-weather-toggle',
