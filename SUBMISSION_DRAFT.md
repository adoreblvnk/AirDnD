# AirDnD — SDTH 2026 Challenge Track Synthesis & Submission Dossier

**Target Event:** Singapore Defense Tech Hackathon (SDTH 2026)  
**Event Link:** [https://luma.com/sdth-2026](https://luma.com/sdth-2026)  
**Organizers:** NUS Enterprise, Defence Venture Lab (DVL), TUM Venture Labs, EDTH  
**Event Dates:** Main Hackathon: 25–27 September 2026 @ NUS Enterprise @ i3  
...
| Evaluation Dimension | How AirDnD Optimizes for the Rubric & Judges |
| :--- | :--- |
...
| **30% Operational Fit (SAF & SG Reality)** | Solves Singapore’s core defense vulnerability: **zero strategic depth (50 km)** and warning times measured in seconds. In a dense Electronic Warfare (EW) environment, central links will be severed. AirDnD eliminates the single point of failure (central C2) while enforcing safety geofencing to prevent kinetic debris over dense HDB residential estates. |
| **30% Venture Incubatability (DVL "Little Red Dome")** | Fits DVL's "Lord of the Rings" prime strategy. AirDnD is the **software-defined autonomous coordination brain** that is hardware-agnostic—slotting onto COTS/indigenous interceptor airframes (Layers 1 & 2) and bridging into existing SAF C2 ecosystems via Cursor-on-Target (CoT). |

---

## 2. Project Positioning & Track Details
* **Team Name:** AirDnD
...
## 3. Challenge Pitch & Technical Specifications

### 🛡️ Team AirDnD | Challenge Track: 03. Interceptors (Layer 3: Onboard Autonomy & Swarm Coordination)
...
* AirDnD decouples strategic authorization from split-second tactical execution through a **two-tiered architecture**:
...
* We will deliver a **live, deterministic 3D software benchmark & tactical digital twin** demonstrating:
  1. **Distributed WTA Auction Convergence:** Real-time P2P target bidding across 20–50 heterogeneous interceptors (FPV quads, fixed-wings, micro-missiles) converging in under 100 milliseconds against a 40–150 threat Mixture of Threats (MoT) salvo (Shaheds, agile FPVs, subsonic cruise missiles).
  2. **EW Jamming Resilience & Link Dropoff:** Telemetry link to ground severed mid-engagement (100% packet loss); the swarm seamlessly degrades to edge consensus over 802.15.4 / Wi-Fi mesh using compact (≤32 bytes) state frames.
  ...
  4. **Collateral Safety Geofencing:** Real-time kinetic intercept corridors calculated over Singapore littoral waters, preventing debris impact on urban HDB zones.

---

**Q4: If it works, what future developments would be needed to make it an operational solution?**
* **Hardware-in-the-Loop (HIL) Micro-Node Validation:** Porting the lightweight edge consensus engine to low-cost embedded companion microcontrollers (ESP32 / 32-bit RISC-V @ 160MHz, 512KB SRAM) on physical bench rigs to prove sub-5ms compute feasibility under memory and power constraints.
